...
|----------|-----------------|-----|
| 1 t-in → 1 Orchard out | max(1,0) + 2 = 3 → grace = 2 | 10,000 |
| 1 t-in → 2 Orchard out | max(1,0) + 2 = 3 → 3 | 15,000 |
| 2 t-in → 1 Orchard out | max(2,0) + 2 = 4 → 4 | 20,000 |
| 1 t-in → 1 t-out + 1 Orchard | max(1,1) + 2 = 3 → 3 | 15,000 |
...
// Example: 1 input, 1 Orchard output + 1 Orchard change
const fee = estimateFee(1, 0, 2);  // 10,000 zatoshis
```
...
```
1 input → 1 Orchard payment
  = 1 transparent input + 2 Orchard outputs (payment + change)
  = max(1, 0) + max(2, 2) = 3 → 3 actions
  = 15,000 zatoshis

// But with grace actions:
  = max(2, 3) = 3 → 15,000 zatoshis
```
...
const fee1 = estimateFee(3, 0, 1);  // 15,000

// More efficient: 1 input (if you have a larger UTXO)
const fee2 = estimateFee(1, 0, 1);  // 10,000
```
...
const fee1 = estimateFee(1, 0, 1) * 2n;  // 20,000

// One transaction with 2 Orchard outputs
const fee2 = estimateFee(1, 0, 2);  // 10,000
```
...
const estimatedFee = 10_000n;       // Typical fee
...
<Tip>
  When selecting UTXOs, add a small buffer (e.g., 10,000 zatoshis) to account for fees and potential change.
</Tip>
